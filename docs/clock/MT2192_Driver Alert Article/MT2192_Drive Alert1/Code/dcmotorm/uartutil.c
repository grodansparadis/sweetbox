...
volatile int CharactersToRead = BUFFER_SIZE;			// Number of characters to be read from UART 1
volatile byte RingBuffer[BUFFER_SIZE];                  // Ring Buffer for incomming data from serial port (UART 1)
...
volatile int NumberCharacters2 = 0;                      // Total number of characters in ring buffer.
volatile int CharactersToRead2 = BUFFER_SIZE;			// Number of characters to be read from UART 2
volatile byte RingBuffer2[BUFFER_SIZE];                  // Ring Buffer for incomming data from serial port (UART 2)

volatile int StartBuffer2=0;                            // Start pointer for ring buffer       
volatile int EndBuffer2=0;                              // End pointer for ring buffer
volatile byte ReceivedDataFlag2 = FALSE;                 // Indicates that data was received into the ring buffer
...
volatile byte TheByte2 = 0;
volatile byte RX_Data_Received = FALSE;
volatile byte RX_Data_Received2 = FALSE;

#ifdef STDIO_INTERRUPTS

// Stdio interrupt code  used for UART with ring buffers
...
/*
	while(DataRdyUART2())                 						//Read the receive buffer as long as at least one or more characters 
															//can be read
	 {
  	( *( Receiveddata2)++)=ReadUART2();  
 	}
*/
...
void            InitializeUart1(unsigned long The_Baud_Rate)
{
    ...
    TxSerialInterruptCount1 = 0;                                   // Reset UART 1 TX Serial Interrupt counter
 	...
 	CharactersToRead = BUFFER_SIZE;                                    // Number of characters to read into buffer
...
/* 
    ...
    // Configure USART 1 interrupts (1 = High Priority, 0 = Low Priority).   
...
//	baud=(((unsigned long)TCY_PER_SECOND)/(16*baudRate))-1; 
  ...
  // Flush the ring buffer
    ...
    FlushBuffer2();
 	...
 	CharactersToRead2 = BUFFER_SIZE;                                    // Number of characters to read into buffer
...
/* 
    ...
    // Configure USART 2 interrupts (1 = High Priority, 0 = Low Priority).   
...
void RxProcessUart1(void)
{
       ...
       LED_1 = ~LED_1;			// Toggle LED 1 indicating UART 1 RX interrupt processed
    	...
    	// First check to see if data is available from the UART 1 and then read it if 
    	// it has been received.  Buffer incoming characters until the buffer is full then set the Rx Data Received flag
        // so that I/O routines can read it.  Also check for Carriage Return which sets the flag immediatly.  

    	if (DataRdyUART1())
    	{
      		// Save byte read from USART into the serial input ring buffer
    		LocalData = ReadUART1();
			if ((LocalData != 0) && (LocalData != -1) )
  			{
 				RingBuffer[EndBuffer] = LocalData;
				...
				if ((NumberCharacters >= CharactersToRead) || (LocalData == CR))
 				{
  					RX_Data_Received = TRUE;
 				}
     		}
...
void RxProcessUart2(void)
{
       ...
       LED_2 = ~LED_2;			// Toggle LED 2 indicating UART 2 RX interrupt processed
    	...
    	// First check to see if data is available from the UART 2 and then read it if 
    	// it has been received.  Buffer incoming characters until the buffer is full then set the Rx Data Received flag
        // so that I/O routines can read it.  Also check for Carriage Return which sets the flag immediatly.  
    	...
    	{
      		// Save byte read from USART into the serial input ring buffer
  			LocalData2 = ReadUART2();
			if ((LocalData2 != 0) && (LocalData2 != -1) )
  			{
  				RingBuffer2[EndBuffer2] = ReadUART2();
 
				EndBuffer2++;
        		EndBuffer2 = EndBuffer2 & (BUFFER_SIZE - 1);        // Wrap the EndBuffer pointer if necessary
        	 
            	NumberCharacters2++;                                   // Increment the number of characters currently in the buffer
          
				if ((NumberCharacters2 >= CharactersToRead2) || (LocalData2 == CR))
 				{
  					RX_Data_Received2 = TRUE;
 				}
     		}
    	}
...
//******************************************************************************
//* TxProcessUart1 -  Process the UART 1 TX interrupt.  
//******************************************************************************
...
//******************************************************************************
//* TxProcessUart2 -  Process the USART 2 TX interrupt.  
//******************************************************************************
...
//******************************************************************************
//* FlushBuffer - Flush the ring buffer for UART1
//******************************************************************************
...
//******************************************************************************
//* FlushBuffer2 - Flush the ring buffer for UART2
//******************************************************************************
...
void    FlushBuffer2(void)
{
	int i;												// Local loop index
  	...
  	StartBuffer2 = 0;                                   // Start pointer for ring buffer       
  	EndBuffer2 = 0;                                     // End pointer for ring buffer
    NumberCharacters2 = 0;                               // Reset total number of characters in ring buffer
   	RX_Data_Received2 = FALSE;
...
//******************************************************************************
//* string2 - Writes a string terminated with a null character from RAM to the
//* serial port UART 2.                               
//******************************************************************************
	...
	do
  	{  
		// Send byte to the Serial display  
 		...
 		send_byte2(*s);                                // Write character directly to the LCD
...
//******************************************************************************
//* get_byte - Get next character from the RS-232 Port (UART 1).  The function 
...
byte	get_byte(void)
{
   ...
   //INTCONbits.GIE = 0;                                    // Disable all interrupts
	...
	Data[1] = 0;
 	...
 	// This code works !!! Wait for character to arrive to UART 1 and read it.
...
//******************************************************************************
//* get_byte2 - Get next character from the RS-232 Port UART 2.  The function 
//* returns a -1 if the routine times out. 
//******************************************************************************
...
/*       
   ...
   while (RingBuffer2[StartBuffer2] == CR) 
	{
		StartBuffer2++;
      StartBuffer2 = StartBuffer2 & (BUFFER_SIZE - 1);    // Wrap the StartBuffer pointer if necessary

 	}
*/
	// Decrement the number of characters currently in the buffer
 	NumberCharacters2--;                                   
    if (NumberCharacters2 < 0)
    {
     	//NumberCharacters2 = 0;
 		RX_Data_Received2 = FALSE;
        Data[0] = 0;
	...
	else
	{
		// There is still data to be read from buffer
   		...
   		//Data = RingBuffer2[StartBuffer2];
   		Data[0] = RingBuffer2[StartBuffer2];

   		StartBuffer2++;
   		StartBuffer2 = StartBuffer2 & (BUFFER_SIZE - 1);    // Wrap the StartBuffer pointer if necessary
	}
    
    //printf("trace 8: ReceivedDataFlag = %d  NumberCharacters2 = %d  Data = !%c' \r\n", ReceivedDataFlag, NumberCharacters2, Data[0]);
   //printf("StartBuffer2 = %d, EndBuffer2 = %d \r\n", StartBuffer2, EndBuffer2);
...
//******************************************************************************
//* get_byte - Get next character from the RS-232 Port (USART 1).  The function 
//* returns a -1 if the routine times out. 
//******************************************************************************
...
byte	get_byte(void)
{
   ...
   return (Data[0]);   	 
    ...
    // First check to see if data is available from the UART and then read it if 
    // it has been received.  
...
byte	get_byte2(void)
{
   ...
   Data[0] = InputBuffer2[InputBufferIndex2++];
   return (Data[0]);   	 
...
//******************************************************************************
//* send_byte - Send character to RS-232 Port (USART1)
//******************************************************************************
...
//******************************************************************************
//* send_byte2 - Send character to RS-232 Port (UART 2)
//******************************************************************************
void            send_byte2(byte c)
{
   	...
   	OutputBuffer2[OutputBufferIndex++] = c;
...
//* Description:      Similar to stdio getch using UART1.                      *   
//*                                                                            *
//******************************************************************************
byte GetByte(void)
{
...
#else
	// This code works !!! It does not wait to read the character 
    while (DataRdyUART1());
...
//* Description:      Similar to stdio getch using UART2.                      *   
//*                                                                            *
//******************************************************************************
byte GetByte2(void)
{
...
#ifdef STDIO_INTERRUPTS
	// This code works !!! Wait for character to arrive to UART 2 and read it.
	while (!RX_Data_Received2);
...
//* Description:      Similarto stdio putch using UART1.                       *   
//*                                                                            *
//******************************************************************************
...
//* Description:      Similarto stdio putch using UART2.                       *   
//*                                                                            *
//******************************************************************************
...
//******************************************************************************
//* getch - Receive a character from the RS-232 port with no echo and   wait ,
//*         instead of directly from keyboard (C40 version).  Returns 0 if no
//*         valid character has been read from UART1 serial port.
//******************************************************************************
int getch(void)
{
   int c;
...
//******************************************************************************
//* getch2 - Receive a character from the RS-232 port with no echo and   wait ,
//*         instead of directly from keyboard (C40 version).  Returns 0 (null) if no
//*         valid character has been read from UART2 serial port.
//******************************************************************************
int getch2(void)
{
   int c;
	...
	//while (!ReceivedDataFlag2);
	
	//if (NumberCharacters2 >= 1 )
	//while (NumberCharacters2 <= 0 );
	//{ 
   	while ((c = (int) get_byte2()) == 0);
	//}
/*
	else
	{
...
*/
  	ReceivedDataFlag2 = FALSE;										// Set data flag to FALSE 
...
//******************************************************************************
//* getch - Receive a character from the RS-232 port with no echo and no wait ,
//*         instead of directly from keyboard (C40 version).  Returns -1 if no
//*         valid character has been read from serial port.
//******************************************************************************
int getch(void)
{
   int c;
...
//******************************************************************************
//* getch2 - Receive a character from the RS-232 port with no echo and no wait ,
//*         instead of directly from keyboard (C40 version).  Returns -1 if no
//*         valid character has been read from serial port.
//******************************************************************************
int getch2(void)
...
long unsigned int ReadHexUnsignedLongInteger(void)
{
    ...
    // Combine the eight digit hex value and convert it to a 32-Bit unsigned word
